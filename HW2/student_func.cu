                   ...
                   const float* const filter, const int filterWidth)
{
	...
	for(int filter_r = -halfWidth; filter_r <= halfWidth; ++filter_r)
	{
		...
		{
			int pixel_r = min(max(thread_2D_pos.y+ filter_r, 0), rows);
			int pixel_c = min(max(thread_2D_pos.x+ filter_c, 0), cols);

			float value = static_cast<float>(inputChannel[pixel_r*numCols+pixel_c]);
			float f_value = static_cast<float>(filter[(filter_r+halfWidth)*filterWidth + filter_c + halfWidth]);
