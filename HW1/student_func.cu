...
{
	int i = blockIdx.y * BLKH + threadIdx.y;
	int j = blockIdx.x * BLKW + threadIdx.x;
    int idx = i*numCols + j;

    uchar4 rgba = rgbaImage[idx];
	//Luma coding for standard television.
    greyImage[idx] = .299f*rgba.x + .587f*rgba.y + .114f*rgba.z;
	//Luma coding for HDTV by ATSC
	//greyImage[idx] = .2126f*rgba.x + .7152f*rgba.y + .0722f*rgba.z;
  //TODO
